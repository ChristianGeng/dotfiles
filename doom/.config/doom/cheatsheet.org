...
Requires ~xclip~ package for terminal Emacs (see config.org CLIPBOARD section).

~xclip-mode~ and Evil registers do *not* interfere:
- ~xclip-mode~ hooks into Emacs' kill-ring ↔ OS clipboard bridge only
- Evil registers (~"a~–~"z~, ~"+~, ~"*~) are a separate layer on top
- With ~xclip-mode~ active, plain ~y~ already syncs to clipboard, making ~"+y~ and ~y~ equivalent in practice
