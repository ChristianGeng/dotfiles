...
:GPTEL_SYSTEM: You are a large language model living in Emacs and a helpful assistant. Respond concisely.\nYou are a line manager preparing to write a report for an annual feedback meeting for a colleague. Focus on their strengths, contributions, areas for improvement, and overall performance. Provide clear, constructive feedback in a professional tone, ensuring it is supportive and encouraging. Avoid any personal judgments and keep the report objective.
:END:
...
- [[#org-mode][ORG MODE]]
  ...
  - [[#org-latex-export--steuerjahr-class][Org LaTeX Export — Steuerjahr class]]
  - [[#org-auto-tangle][Org-auto-tangle]]
  - [[#org-babel-mermaid-ob-mermaid][Org-Babel Mermaid (ob-mermaid)]]
- [[#perspective][PERSPECTIVE]]
- [[#project][Project]]
- [[#per-host-appearance][Per-host appearance]]
...
- [[#pass---manage-secrets-using-the-default-unix-password-manager][PASS - Manage Secrets using the default unix password manager]]
  ...
  - [[#pass---environment-variables-management][PASS - Environment Variables Management]]
...
#+begin_example bash
doom sync      # reload configuration aka sync,
doom build     # build after syncing
...
~user-emacs-directory~ is "/home/cgeng_local/doom-emacs/.local/cache/" in doom emacs.
...
Probes three directories, helpful for machines without dropbox.
...
** Emacs Server
Start the Emacs server so emacsclient can connect (used by org-roam MCP, shell scripts, etc.).
...
*** System Integration with xdg-mime
To make system applications (Chrome, file managers, etc.) open files with emacsclient:
...
MimeType=text/plain;text/x-makefile;text/x-python;text/x-shellscript;
...
To open files in the currently active Emacs window (same frame) instead of creating a new frame:

#+begin_src bash :tangle no
# 1. Create wrapper script
mkdir -p ~/.local/bin
cat > ~/.local/bin/emacsclient-same-window << 'EOF'
#!/bin/bash
# Open file in existing Emacs window (same frame)
emacsclient --eval "(find-file \"$1\")" || emacs "$1" &
EOF
chmod +x ~/.local/bin/emacsclient-same-window

# 2. Create desktop entry
mkdir -p ~/.local/share/applications
cat > ~/.local/share/applications/emacsclient-same-window.desktop << 'EOF'
[Desktop Entry]
Name=Emacs Client (Same Window)
GenericName=Text Editor
Comment=Edit in existing Emacs window
MimeType=text/plain;text/x-makefile;text/x-python;text/x-shellscript;
Exec=~/.local/bin/emacsclient-same-window %f
Icon=emacs
Type=Application
Terminal=false
Categories=Utility;TextEditor;
...
# 3. Set as default text editor
xdg-mime default emacsclient-same-window.desktop text/plain

# 4. Verify
grep "text/plain=" ~/.config/mimeapps.list
#+end_src

*** Managing Both Options

If both desktop entries are installed, you can switch between them:

#+begin_src bash :tangle no
# Check which is currently default
xdg-mime query default text/plain

# Switch to new frame mode
xdg-mime default emacsclient.desktop text/plain

# Switch to same window mode
xdg-mime default emacsclient-same-window.desktop text/plain

# Remove one you don't want (e.g., remove new frame version to keep only same-window)
rm ~/.local/share/applications/emacsclient.desktop
# or
rm ~/.local/share/applications/emacsclient-same-window.desktop
#+end_src
...
* CLIPBOARD
System clipboard integration that works in GUI, local terminal, and terminal over SSH.
- GUI: =select-enable-clipboard= (Emacs default)
- Local terminal: =xclip= bridges kill-ring to OS clipboard via X11
- SSH terminal: =clipetty= uses OSC 52 escape sequences to set the *local* machine's clipboard
  (requires a modern terminal: iTerm2, Kitty, WezTerm, Alacritty, foot, etc.)

Requirements: =sudo apt install xclip= on local machines with X11.
...
** Org file apps
Open pdfs and images with xdg-open instead of inside Emacs.
...
** Org LaTeX Export — Steuerjahr class

Custom LaTeX class =steuerjahr= für Steuerübersichten. Aktivieren mit
=#+LATEX_CLASS: steuerjahr= im org-file, dann =C-c C-e l p= zum Exportieren.
...
** Org-Babel Mermaid (ob-mermaid)
=ob-mermaid= enables rendering [[https://mermaid.js.org/][Mermaid]] diagrams from =#+begin_src mermaid= blocks in org-mode.
Evaluate a block with =C-c C-c= to produce a PNG inline in the buffer.

*** Prerequisites

Requires the =mmdc= CLI from the [[https://github.com/mermaid-js/mermaid-cli][@mermaid-js/mermaid-cli]] npm package.
...
If you use nvm, ensure the nvm node version is active when Emacs starts, or set the
full path to =mmdc= explicitly (see config below).

*After adding to =packages.el=, sync Doom:*
#+begin_src sh :tangle no
doom sync
#+end_src
...
=executable-find= resolves =mmdc= from your =$PATH= at load time.
If Emacs does not inherit your shell =$PATH= (common on macOS / systemd-launched Emacs),
...
- =:file= path is relative to the org file's directory
- =C-c C-c= renders the block and inlines the PNG
- =C-c C-v v= views the result
...
What happens right after switching projects (~SPC p p~). Only the
non-default settings are listed; doom already sets =projectile-indexing-method=,
caching, and the completion system sensibly.
...
Recursively register every Git repo under a root as a Projectile known project
(=M-x cg/projectile-add-projects-under=). On the demo box, point it at
=~/project/cgeng/work/research/iva= (or =.../work=) to pick up all of =demos/=
and =tools/= in one go. It prunes at each repo, so heavy trees (=node_modules=,
=.venv=) and submodules aren't walked, and persists the list with
...
A colored host badge in the modeline — the emacs counterpart to the kitty
per-host background tint, so it's obvious "which box am I on".

Keyed on =(system-name)=, i.e. the *real* hostname — NOT the ssh alias kitty
matches. Check it per host with =M-: (system-name)= and adjust the regexps:
the p4d login node reports =ip-10-1-127-78= (matched by =^ip-10-1-=), and the
demo box reports =ip-172-31-2-123= (matched by =^ip-172-31-=) — NOT the ssh
aliases =iva-p4d-param= / =iva-M2-demo-cgeng=. The hostname is the *private* one;
an Elastic IP only fixes the public endpoint, not =(system-name)=.

The badge is surfaced via =global-mode-string= (rendered by doom-modeline's
=misc-info= segment). We deliberately do NOT redefine the =main= modeline layout:
hard-coding the segment list breaks on any host whose doom-modeline version
lacks one of those segments (symptom: a completely blank modeline). Going
through =global-mode-string= is version-proof.
...
[[https://www.perplexity.ai/search/vertico-versus-helm-mWKEYuh9T4C_TujxKjqsgQ#2][My perplexity thread]] gave me this suggestion:
...
#+RESULTS:
: helm-swoop-via-consult-line-at-point
...
Built-in keybindings (TAB, C-j) can be used to jump/to the right place!
...
#+RESULTS:
: consult-projectile-ripgrep-symbol-at-point
...
- Use C-c C-c to save you changes, which will be promulgated to all files you’ve chosen
...
*** 1. Secret Mapping Configuration

This section defines how pass entries map to environment variables.
The format is: (ID :pass "pass/path" :env "ENV_VAR_NAME")
...
Environment variables are automatically set on Emacs startup via ~pass-simple-export-env~.

Access them in Emacs with:
...
To access these variables in your shell, generate a script:


# (cg/write-pass-export-script "~/.emacs.d/pass-env.sh")


Then add this to your ~/.bashrc or ~/.zshrc:

# [ -f ~/.emacs.d/pass-env.sh ] && source ~/.emacs.d/pass-env.sh


*** 4. Managing Secrets

You have two options to update your pass store:

1. **From Encrypted File** (Recommended for manual updates):
   - Edit: =~/.config/doom/secrets.el.gpg=
   - Run: ~M-x cg/pass-bulk-insert-from-file~
   - This uses the =cg/private-pass-secrets= variable by default

2. **From Variable** (For programmatic use):
   ```elisp
   (cg/pass-bulk-insert-from-var
    '(("code/anthropic_api_key" . "your-secret-here")
      ("code/xai_api_key" . "another-secret")))
   ```

3. After updating, restart Emacs or run ~(cg/init-secrets)~ to update environment

*** 5. GPG Secret File Maintenance

When updating the GPG-encrypted secrets file:
1. Decrypt: ~gpg -d ~/.config/doom/secrets.el.gpg > secrets.el~
2. Edit: ~emacs secrets.el~
3. Re-encrypt: ~gpg -e -r your-email@example.com -o ~/.config/doom/secrets.el.gpg secrets.el~
4. Update pass store: ~M-x cg/pass-bulk-insert-from-file~
5. Securely delete: ~shred -u secrets.el~
...
| AIDER COMMAND                 | WHAT IS DOES                    |
|-------------------------------+---------------------------------|
| /model openai/gpt-5           | use gpt-5 as the defaulat model |
| /weak-model openai/gpt-5-mini | use gpt5-mini-as the weak model |
| /editor-model openai/gpt-5    |                                 |

/editor-model

| Architect/Main Model | Weak Model               | Editor Model    | Provider  | Notes                          |
|----------------------+--------------------------+-----------------+-----------+--------------------------------|
| Claude 3 Opus        | Claude 3.5 Haiku         | Claude Sonnet 4 | Anthropic | Strong edits, cheap summarizer |
| Claude Sonnet 4      | Claude 3.5 Haiku         | Claude Sonnet 4 | Anthropic | Balanced cost/quality          |
| GPT-4o               | GPT-3.5                  | GPT-4o          | OpenAI    | Optimal for most workflows     |
| GPT-4 Turbo          | GPT-3.5                  | GPT-4 Turbo     | OpenAI    | Efficient for large projects   |
| GPT-5 (planned)      | GPT-3.5 or GPT-5 (cheap) | GPT-5           | OpenAI    | Use when available             |
