...
** Securely loading API keys with pass (password-store)

Use Emacs's `password-store` integration to read secrets from your system password store (pass) instead of hardcoding keys.

- __What is `password-store-get`?__ It reads a secret from pass. Example: `(password-store-get "code/openai_api_key")` returns the value stored at that pass path.
...
The "key ID" is the identifier for your GPG key. List your keys with a long key id:
...
- __Key ID__: the part after the slash (`ABCDEF1234567890`). You can also use your key's email (UID) instead of the ID in the next steps.
...
Initialize the password store with your key (use the long key ID or your GPG email):

#+begin_src sh
pass init ABCDEF1234567890   # or: pass init your.email@example.com
#+end_src

*** Store your OpenAI API key in pass
...
- Doom module (recommended): enable `:tools pass` in `~/.config/doom/init.el`, then run `doom sync` and restart Emacs.
- Or install the `password-store` package yourself if not using the Doom module.

*** Use it in your config

Replace the environment line in the Aider hook with `password-store-get`:
...
*** Alternative: auth-source (~/.authinfo.gpg)

Store your key in an encrypted authinfo file:
...
#+RESULTS:
: #[nil ((with-current-buffer (get-buffer-create "*AI Global Rules*") (erase-buffer) (insert my/ai-global-rules) (markdown-mode) (switch-to-buffer (current-buffer)))) nil nil nil nil]
