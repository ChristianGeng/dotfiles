...
* Main Script
  ...
  #+begin_src bash :tangle ./bash/.shell_scripts :mkdirp yes
     ...
     function generate_local_configs()
     {
         # Generate ~/.local_configs from pass store
         # Usage: generate_local_configs [--dry-run]
         local script_dir="$(dirname "${BASH_SOURCE[0]}")"
         local gen_script="$script_dir/../scripts/generate-local-configs"
         
         if [ -f "$gen_script" ]; then
             "$gen_script" "$@"
         else
             echo "Error: generate-local-configs script not found at $gen_script"
             return 1
         fi
     }
...
This might not work and maybe we need fuser:
https://stackoverflow.com/questions/39322089/node-js-port-3000-already-in-use-but-it-actually-isnt

- npx kill-port 3000 does not work
- sudo lsof -i:<PORT_NO> will find ports also
- fuser often is the last resort

  fuser -k PORT-NO/tcp
  fuser -k PORT-NO/tcp
  fuser -n tcp -k 3000

 TODO: Replace/supplement killport with fuser for tough cases
...
** PDF password protection / unprotection

Passwort-Schutz und -Entfernung für PDFs.
Passwort liegt im =pass=-Store unter =personal/passwords/vlh-pdf-secret=
(siehe [[file:CREDENTIAL_MANAGEMENT.md][CREDENTIAL_MANAGEMENT.md]]).

Optionaler zweiter Parameter: abweichender pass-Pfad (für andere Dokumente/Passwörter).

Voraussetzungen:
- =pdftk= — Passwort setzen: =sudo apt install pdftk=
- =qpdf= — Passwort entfernen: =sudo apt install qpdf=
- =pass= verfügbar

Verwendung:
#+begin_src bash :eval no
# Mit Standard-Passwort (VLH)
password_protect   steuerbescheid.pdf
password_unprotect steuerbescheid_protected.pdf

# Mit abweichendem pass-Eintrag
password_protect   anderes.pdf   personal/passwords/other-secret
password_unprotect anderes_protected.pdf personal/passwords/other-secret
#+end_src

Das Passwort wird *zur Laufzeit* via =pass show= abgerufen — sitzt nie im Environment.

#+begin_src bash :tangle ./bash/.shell_scripts :mkdirp yes
_pdf_get_password() {
    # Usage: _pdf_get_password [pass-path]
    # Falls back to personal/passwords/vlh-pdf-secret
    local pass_path="${1:-personal/passwords/vlh-pdf-secret}"
    local pw
    pw=$(pass show "$pass_path" 2>/dev/null)
    if [[ -z "$pw" ]]; then
        echo "Error: could not retrieve password from pass ($pass_path)" >&2
        return 1
    fi
    echo "$pw"
}
...
password_protect() {
    # Usage: password_protect <file.pdf> [pass-path]
    # Requires: pdftk
    local input="$1"
    local pass_path="$2"
    [[ -z "$input" ]] && { echo "Usage: password_protect <file.pdf> [pass-path]"; return 1; }

    local ext="${input##*.}"
    [[ -z "$ext" || "$ext" == "$input" ]] && { echo "No extension found"; return 1; }

    local abspath
    abspath=$(readlink -e "$input")
    [[ -z "$abspath" ]] && { echo "Could not resolve absolute path"; return 1; }

    local pdf_password
    pdf_password=$(_pdf_get_password "$pass_path") || return 1

    local real_dir filebase filename_wo_ext outpath
    real_dir=$(dirname "$abspath")
    filebase=$(basename "$abspath")
    filename_wo_ext="${filebase%.*}"
    outpath="${filename_wo_ext}_protected.${ext}"

    (
        cd "$real_dir"
        echo "pwd: $PWD"
        echo "pdftk $filebase output $outpath user_pw ***"
        pdftk "$filebase" output "$outpath" user_pw "$pdf_password"
    )
}

password_unprotect() {
    # Usage: password_unprotect <file.pdf> [pass-path]
    # Requires: qpdf
    local input="$1"
    local pass_path="$2"
    [[ -z "$input" ]] && { echo "Usage: password_unprotect <file.pdf> [pass-path]"; return 1; }
    ...
    filebase=$(basename "$abspath")
    filename_wo_ext="${filebase%.*}"
    outpath="${filename_wo_ext}_decrypted.${ext}"

    (
        cd "$real_dir"
        echo "pwd: $PWD"
        echo "qpdf --decrypt $filebase $outpath"
        qpdf --password="$pdf_password" --decrypt "$filebase" "$outpath"
    )
}
#+end_src
...
convert_webm_to_ogg() {
    local inputfile="$1"
    if [[ -z "$inputfile" ]]; then
        echo "Usage: convert_webm_to_ogg <inputfile>"
        return 1
    fi
    ffmpeg -i "$inputfile" -vn -c:a libvorbis -qscale:a 6 "$(basename "$inputfile" .webm)".ogg
}
