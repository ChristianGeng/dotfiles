...
** PDF password protection (pdftk)

Schützt eine PDF-Datei mit einem User-Passwort via =pdftk=.
Das Passwort liegt im =pass=-Store unter =personal/passwords/vlh-pdf-secret=
(siehe [[file:CREDENTIAL_MANAGEMENT.md][CREDENTIAL_MANAGEMENT.md]]).

Voraussetzung: =pdftk= installiert (=sudo apt install pdftk=), =pass= verfügbar.

Verwendung:
#+begin_src bash :eval no
password_protect steuerbescheid.pdf
# → erzeugt steuerbescheid_protected.pdf im gleichen Verzeichnis
#+end_src

Das Passwort wird *zur Laufzeit* via =pass show= abgerufen — es sitzt nie im
Environment und wird nicht gespeichert.

#+begin_src bash :tangle ./bash/.shell_scripts :mkdirp yes
password_protect() {
    # Usage: password_protect <file.pdf>
    # Requires: pdftk, pass store entry personal/passwords/vlh-pdf-secret
    local input="$1"
    [[ -z "$input" ]] && { echo "Usage: password_protect <file.pdf>"; return 1; }
    ...
    local filebase
    filebase=$(basename "$abspath")
    local filename_wo_ext="${filebase%.*}"
    local outpath="${filename_wo_ext}_protected.${ext}"

    local pdf_password
    pdf_password=$(pass show personal/passwords/vlh-pdf-secret 2>/dev/null)
    if [[ -z "$pdf_password" ]]; then
        echo "Error: could not retrieve password from pass (personal/passwords/vlh-pdf-secret)"
        return 1
    fi
